  ...
  return (
    <div className="panel active">
      <div className="period-toolbar">
        <button className={`period-mode ${mode === 'weeks' ? 'active' : ''}`} onClick={() => setMode('weeks')}>
          {t.btnWeeks}
        </button>
        <button className={`period-mode ${mode === 'months' ? 'active' : ''}`} onClick={() => setMode('months')}>
          {t.btnMonths}
        </button>
        <span>
          {mode === 'months' ? t.periodHintMonths(groups.length) : t.periodHintWeeks(groups.length)}
        </span>
      </div>
            ...
            return (
              <div className="period-card" key={group.key}>
                ...
                <div className="period-line">
                  ...
                  <strong>
                    {fmtWeight(start.gewicht)} → {fmtWeight(end.gewicht)} kg
                  </strong>
                </div>
                <div className="period-line">
                  <span>
                  ...
                  <strong className={change <= 0 ? 'green' : 'yellow'}>
                    {change >= 0 ? '+' : ''}{fmtWeight(change)} kg
                  </strong>
                </div>
              </div>
            );
